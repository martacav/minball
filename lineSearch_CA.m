function [x_new, flag] = lineSearch_CA (x, Q, R, p)
...
omTmp = omega;
omTmp(omTmp<-epsTol | omTmp>epsTol)=Inf; 
piTmp = pi;
piTmp(piTmp<-epsTol | piTmp > epsTol)=Inf; 
...
alphas(2) = facetIntersection_CA (Q2, R2, flags(2), m, n, d, p, pm, x);

ratiosNeg = ratios;
ratiosNeg(~(pi <= epsTol & omega < 0)) = -Inf;
[maxRNeg, flags(3)] = max(ratiosNeg);
if maxRNeg > 0
    alphas(3) = facetIntersection_CA (Q2, R2, flags(3), m, n, d, p, pm, x);
end


%4. New solution and info about update S
alphas(alphas < -epsTol) = Inf; 
[alpha, jj] = min(alphas);
flag = flags(jj);
x_new = x+alpha*d;
